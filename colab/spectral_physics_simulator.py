...
==============================
Physically-informed synthetic hyperspectral pixel generator.

WHY SIMULATED DATA:
This sandbox has no internet egress, so real Pixxel L2A cubes, AVIRIS/Hyperion
public scenes, or the USGS Spectral Library cannot be downloaded here. Instead
of faking numbers, this module builds spectra from *real, documented physical
absorption features* (wavelength positions, relative depths, FWHM) so that:
  1. The synthetic dataset is scientifically defensible, not arbitrary noise.
  2. The SAE has genuine, learnable structure to decompose (this matters —
     an SAE trained on pure noise learns nothing interpretable).
  3. Swapping in a real datacube later is a one-function change (see
     `load_real_envi_cube()` stub at the bottom) — everything downstream
     (SAE, classifier, finance layer) is agnostic to where the spectra came from.

Band grid: 400-2500 nm at 5 nm steps -> 421 bands, matching the VNIR-SWIR
range Pixxel's Fireflies/Firefly hyperspectral constellation and AVIRIS-class
sensors operate in (Pixxel's actual bands are narrower, ~5-6nm resolution
across ~150-250 usable bands after removing water-vapor absorption gaps —
we keep the full 421 for realism and let the model discover which bands
matter).

Physical basis for each class's absorption features (wavelength, depth, FWHM
in nm) — these are the real, published positions from vegetation spectroscopy
(chlorophyll, water, cellulose/lignin, red-edge) and USGS mineral spectral
library band positions (kaolinite doublet, alunite, iron oxide, montmorillonite/
lithium-bearing clay proxy):
    - Chlorophyll-a absorption:      680 nm
    - Chlorophyll blue absorption:   480 nm
    - Red edge inflection:           700-725 nm (sigmoid rise)
    - Leaf water bands:              970, 1200, 1450, 1940 nm
    - Cellulose/lignin (dry matter): 2100, 2280 nm
    - Iron oxide (ferric) charge transfer: 870 nm broad absorption
    - Kaolinite Al-OH doublet:       2160, 2208 nm (narrow, diagnostic)
    - Alunite:                       1480, 2170, 2320 nm
    - Li-bearing clay/spodumene proxy (Al-OH + Li-OH shift): 2200, 2340 nm
      (spodumene itself is largely featureless in reflectance; we use the
      montmorillonite/mica Li-bearing clay alteration halo, which IS the
      real remote-sensing proxy explorationists use for pegmatite lithium)
    - Urban concrete/impervious:     broad, flat, low-contrast albedo
"""
